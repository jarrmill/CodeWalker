...
import {
  ...
  loadPastedDiffTool,
} from '../tools/orientation-grader-tool';
...
- Use githubGetPullRequestTool to fetch a specific pull request by number. It returns the title, description, and unified diff, and loads that change as the session's review context for grading. If the diff was truncated, say so and keep that limitation in mind.
- Use loadPastedDiffTool when the developer pastes a raw diff directly instead of choosing a PR. Call it once with the pasted diff (and any description they gave) to load that change as the session's review context. After that, grade normally.
- Use gradeOrientationStageTool to grade the developer's explanation for the current stage. Do NOT judge understanding yourself — always call this tool once the developer has explained a stage, and let its verdict decide whether to advance. Pass the matching stage id (select-task, understand-context, understand-rationale), the developer's explanation verbatim, and the notes confirmed in earlier stages as priorNotes. The diff and PR description are supplied automatically from the loaded change — do NOT pass them.

## Flow
1. If no change is loaded yet, help the developer find and load one: list open PRs, fetch the one they name with githubGetPullRequestTool, or register a pasted diff with loadPastedDiffTool.
...
The diff and description are loaded into the session when the change is first fetched or pasted, and the grader reads them automatically — you do not carry or pass them yourself. Do not re-fetch the pull request on every turn just to grade. If a grade call reports that no change is loaded, load it again (fetch the PR, or ask the developer to re-paste the diff) and retry.
